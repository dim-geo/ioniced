...
{
  printf
    ("ioniced %s: daemon to automagically update priority/nice-level of running programs.\n",
     IONICED_VERSION);
  printf (" [-h]\t\tshow this help\n"
	  " [-v]\t\tbe more verbose\n"
	  " [-n]\t\tstay in foreground (no daemon)\n"
	  " [-u sec]           update interval in seconds\n"
	  " [-c config_file]   load given config file (default: /etc/ioniced.conf)\n"
...
main (int argc, char *argv[])
{
  ...
  char *pid_file = NULL;
  uid_t current_uid;		/* uid of ioniced itself */
  ...
  if ((current_uid = getuid ()) != 0)
  {
    ...
    printf ("Only programs with UID=%i will be touched by ioniced.\n\n",
	    current_uid);
  }
  ...
  if (config_file == NULL)
  {
    ...
    if (current_uid == 0)
      config_file = "/etc/ioniced.conf";
    else
    {
      /* non-root user get it from home */
      snprintf (buf, 128, "%s/.ioniced.conf", getenv ("HOME"));
      config_file = buf;
    }
  }
  ...
  /* verbose into syslog */
  msg ("Ready: version %s", IONICED_VERSION);
  ...
  while (1)
  {
	...
	if (config_entry != NULL)
	{
	  ...
	  /* run ionice */
	  snprintf (buf, 1023, "ionice %s -p %i",
	  ...
	  if (system (buf))
	  {
	    ...
	    {
	      msg
		("Error! Running \"ionice %s\" on %s (pid: %i) failed.",
